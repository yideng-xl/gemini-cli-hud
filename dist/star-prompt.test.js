...
import fs from 'fs';
import { loadStarState, saveStarState, recordSession, markPrompted, shouldShowStarPrompt, renderStarPrompt, SESSION_THRESHOLD, } from './star-prompt.js';
vi.mock('fs');
beforeEach(() => {
    vi.resetAllMocks();
});
describe('loadStarState', () => {
    ...
    it('should parse existing state file', () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ sessionCount: 3, prompted: false }));
        const state = loadStarState();
...
describe('saveStarState', () => {
    it('should write state as JSON', () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('hud-star.json'), expect.stringContaining('"sessionCount": 5'));
    });
...
describe('recordSession', () => {
    it('should increment session count', () => {
        vi.mocked(fs.existsSync).mockReturnValue(true);
        vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ sessionCount: 4, prompted: false }));
...
describe('markPrompted', () => {
    it('should set prompted to true', () => {
        vi.mocked(fs.existsSync).mockReturnValue(true);
        vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ sessionCount: 5, prompted: false }));
        markPrompted();
