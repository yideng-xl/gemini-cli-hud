...
import { parseChecklist, parseSteps, extractTaskProgress, updateTaskProgress, createInitialTaskProgress, toolBasedProgress, formatTaskModule, } from './task-utils.js';
...
describe('formatTaskModule', () => {
    ...
    it('should format in-progress tasks', () => {
        const progress = { total: 5, completed: 2, items: [], source: 'checklist', lastUpdated: Date.now() };
    ...
    it('should show check mark when all tasks done', () => {
        const progress = { total: 3, completed: 3, items: [], source: 'checklist', lastUpdated: Date.now() };
        const result = formatTaskModule(progress);
        expect(result).not.toBeNull();
    ...
    it('should format in Chinese', () => {
        const progress = { total: 4, completed: 1, items: [], source: 'checklist', lastUpdated: Date.now() };
