...
interface StatusLineAPI {
  draw: (content: string) => void;
}

interface GeminiUI {
  statusLine?: StatusLineAPI;
}
  ...
  return {
    action: async (args: GeminiArgs) => {
      ...
      try {
        ...
        args.gemini.ui.statusLine.draw(
          `{magenta}[Gemini]{/magenta} {gray}|{/gray} {cyan}Context: ${bar} ${percent}% (${used}/${total}){/cyan}`,
        );
