...
import {
  ...
  renderQuotaHint,
  SESSION_THRESHOLD,
  QUOTA_HINT_THRESHOLD,
  type PromptState,
} from './star-prompt.js';

vi.mock('fs');

const DEFAULT_STATE: PromptState = { sessionCount: 0, prompted: false, quotaHintShown: false };
...
describe('loadStarState', () => {
  ...
  it('should parse existing state file', () => {
    ...
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({ sessionCount: 3, prompted: false, quotaHintShown: false }),
    );
    const state = loadStarState();
...
describe('saveStarState', () => {
  it('should write state as JSON', () => {
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      expect.stringContaining('hud-prompts.json'),
...
describe('recordSession', () => {
  it('should increment session count', () => {
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({ sessionCount: 4, prompted: false, quotaHintShown: false }),
    );
...
describe('markPrompted', () => {
  it('should set prompted to true', () => {
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({ sessionCount: 5, prompted: false, quotaHintShown: false }),
    );
    markPrompted();
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      expect.any(String),
...
describe('markQuotaHintShown', () => {
  it('should set quotaHintShown to true', () => {
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({ sessionCount: 15, prompted: true, quotaHintShown: false }),
    );
    markQuotaHintShown();
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      expect.any(String),
      expect.stringContaining('"quotaHintShown": true'),
    );
  });
});
