...
import { getMemoryUsage, formatMemoryModule, type MemoryInfo } from './memory-utils.js';
...
describe('formatMemoryModule', () => {
  it('uses green for percent <= 70', () => {
    const info: MemoryInfo = { usedGB: 5.6, totalGB: 16.0, percent: 35 };
  ...
  it('uses yellow for percent > 70', () => {
    const info: MemoryInfo = { usedGB: 12.3, totalGB: 16.0, percent: 77 };
    const mod = formatMemoryModule(info);
  ...
  it('uses red for percent > 90', () => {
    const info: MemoryInfo = { usedGB: 15.0, totalGB: 16.0, percent: 94 };
    const mod = formatMemoryModule(info);
